...
-- Utl Functions
local function getRecordLength(
...
local function dijkstra(graph: any, start: any, target: any)
	...
	local function recursiveStepGraph()
		local culledDistance: DistanceHashmap = filterHashmap(
			distance,
			function(key, _value)
		...
		local unvistedNeighbors =
			getUnvisitedNeighbors(currentNode) :: { [number]: { Node: any, Weight: number } }
	...
	for i = #reversePath, 1, -1 do
		local waypoint = {
			...
			Edge = i > 1 and graph:GetEdgeForNodes(reversePath[i], reversePath[i - 1])
				or false,
		}
